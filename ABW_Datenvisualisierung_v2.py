...
def load_timeseries(
    ...
    # Kein Cache oder Cache leer → aus Postgres laden (Originalcode)
...
with st.sidebar:
    ...
    # Lokaler Cache Schalter
    use_local_cache = st.checkbox("⚡ Use local cache (DuckDB)", value=False,
    ...
    with cols[0]:
        if st.button("🔃 Sync meta → cache", help="Load nodes_hierarchy from Postgres into local cache"):
            m = load_meta(); sync_meta_to_cache(m); st.toast("Metadaten in Cache aktualisiert")
    with cols[1]:
    ...
    with cols[2]:
        ...
        if stats.get("enabled"):
            st.caption(f"Cache: meta={stats.get('nodes_hierarchy_cache',0)} hist={stats.get('nodes_history_cache',0)} env={stats.get('nodes_history_env_cache',0)}")

    st.header("Filters")    # --- Kaskadierende Auswahllisten (Gebäude → Stockwerk → Area)
    ...
    sel_types = st.multiselect("Sensor type", types, help="z. B. ems_desk, ers_eye, ers_co2 …")
    ...
    # Channel-Auswahl (optional) – unabhängig von Typen, damit nur Gebäude/Stockwerk/Area die Verfügbarkeit steuern
    ...
    # Robust: Streamlit kann 1 Datum (einzeln) oder 2 Werte (Range) liefern
...
if run:
    ...
    with st.spinner("Loading time series from Postgres…"):
        channels_to_use = sel_channels if sel_channels else ["occupancy","motion","tempc","hum","co2","light"]
        df = load_timeseries(id_list, channels_to_use, start_dt, end_dt)
